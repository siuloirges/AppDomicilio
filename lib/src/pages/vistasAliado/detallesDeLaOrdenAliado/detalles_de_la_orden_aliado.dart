...
import 'package:traelo_app/src/pages/vistasAliado/detallesDeLaOrdenAliado/respDetallePedido.dart';
...
class DetallesDeOrdenAliado extends StatefulWidget {
  @override
  _DetallesDeOrdenAliadoState createState() => _DetallesDeOrdenAliadoState();
}

class _DetallesDeOrdenAliadoState extends State<DetallesDeOrdenAliado> {
  ...
  Widget build(BuildContext context) {
    ...
    if(element!=null)
    {
    id = element['orden']?.id?.toString();
    }
      // print('**************' + element['orden'].id.toString());
    ...
    return 
      ...
      Scaffold(
          ...
          body: SingleChildScrollView(
            child:element!=null? Column(
              children: [
                tarjetaAliado(context,nombreSucursal: element['orden']?.sucursal?.nombre,direccionSucursal: element['orden']?.sucursal?.direccion),
                ...
                tarjetaFormaDePago( context: context, precio: '${element['orden'].precioTotal}', formaDePago: '${element['orden'].metodoDePago}'),
  ...
  Widget tarjetaPedido({BuildContext context, String producto, String precio, String cantidad}) {
    int precioTotalLocal=int.parse(precio)*int.parse(cantidad);
    final size = MediaQuery.of(context).size;
    final orientacion = MediaQuery.of(context).orientation;
                ...
                children: <Widget>[
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      Text(
                      'Total: \$' + formatter.format(int.parse(precioTotalLocal.toString())),
                        style: TextStyle(fontSize: size.width < 200 ? 10 : 12),
                      ),
                    ],
                  ),
  ...
  Widget tarjetaFormaDePago({BuildContext context, String precio, String formaDePago}) {
    final size = MediaQuery.of(context).size;
    final orientacion = MediaQuery.of(context).orientation;
...
cambiarEstadoAautorizada(BuildContext context ,int id,)async{
  try {
  load(context);
        ...
        children: [
          ...
          Container(
            ...
            child: RaisedButton(
                ...
                onPressed: () {
                  // Navigator.pop(context);
                    cambiarEstadoAautorizada(context,element['orden'].id);
...
//   }
  ...
  put(BuildContext context) async {
    try {
      ...
      load(context);
      dynamic resp = await _http.putArgumentMethod(
          context: context,
  ...
  pedir(BuildContext context) async {
    ...
    try {
      List<Widget> widgets = [];
      // load(context);
      dynamic resp = await _http.getMethod(
        context: context,
      ...
      if (resp['message'] == "true") {
        final model = respDetallePedioFromJson(resp['resp']);
