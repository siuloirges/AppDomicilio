...
class NuevaSucursal extends StatefulWidget {
  ...
  @override
  _NuevaSucursal createState() => _NuevaSucursal();
}

class _NuevaSucursal extends State<NuevaSucursal> {
  File imagenPerfil;
  File imagenPortada;
// dynamic element;
...
//  DireccionesProvider direcciones;
                  ...
                  child: Container(
                      ...
                      child: imagenPerfil != null
                          ...
                          : element['data'] != null
                              ? ClipRRect(
                                  borderRadius: BorderRadius.circular(100),
                                  child: element['data'].urlFotoPerfil != null
                                      ? Image(
                                          fit: BoxFit.cover,
                                          image: NetworkImage(storage +
                                              element['data'].urlFotoPerfil))
                                      : Container(),
                                )
                              : Container()),
                ),
                  ...
                  child: Container(
                      ...
                      child: imagenPortada != null
                          ...
                          : element['data'] != null
                              ? ClipRRect(
                                  borderRadius: BorderRadius.circular(10),
                                  child: element['data'].urlFotoPortada != null
                                      ? Image(
                                          fit: BoxFit.cover,
                                          image: NetworkImage(storage +
                                              element['data'].urlFotoPortada))
                                      : Container(),
                                )
                              : Container()),
                ),
  ...
  Future cargarImageGalery(foto) async {
    var tempImage = await ImagePicker.pickImage(
        maxHeight: 440, maxWidth: 720, source: ImageSource.gallery);
    setState(() {
  ...
  Future cargarImageCamera(foto) async {
    var tempImageCamera = await ImagePicker.pickImage(
        maxHeight: 440, maxWidth: 720, source: ImageSource.camera);
...
// return object of type Dialog
  ...
  _submit() async {
    if (imagenPerfil == null && imagenPortada == null) {
      return alerta(context,
          code: false, contenido: 'Las imagenes son necesarias');
    }
    ...
    Map _respuesta = await http.postMethod(
        ...
        body: {
          ...
          "url_foto_perfil": imagenPerfil != null
              ? "${base64Encode(imagenPerfil.readAsBytesSync())}"
              : "no",
          "url_foto_portada": imagenPortada != null
              ? "${base64Encode(imagenPortada.readAsBytesSync())}"
              : "no",
    ...
    if (_respuesta['message'] == "true") {
      ...
      if (_respuesta['message'] == "true") {
        ...
        try {
          ...
          Navigator.pop(context);
          return alerta(context,
              code: false, contenido: 'Fallo en el modelo: $e');
        }
  ...
  _submitUpDate() async {
    ...
    if (_respuesta['message'] == "true") {
      ...
      if (_respuesta['message'] == "true") {
        var respModel = respSucursalFromJson(_respuesta['resp']);
        Navigator.pop(context);
        Navigator.pop(context);
        Navigator.pop(context);
        Navigator.of(context).pushNamed('SucursalAliado',
